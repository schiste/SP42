...
run_step "focused cargo check" \
  "$CARGO_BIN" check -p sp42-core -p sp42-server -p sp42-cli -p sp42-devtools -p sp42-desktop -p sp42-reporting

run_step "focused cargo test" \
  env RUST_TEST_THREADS="${RUST_TEST_THREADS:-1}" \
  "$CARGO_BIN" test -p sp42-core -p sp42-server -p sp42-cli -p sp42-devtools -p sp42-desktop -p sp42-reporting
