...
use parsoid::ImmutableWikicode;
use parsoid::prelude::*;
use sp42_core::{
    ParsoidBlock, WikiConfig, WikitextEditOutcome, WikitextEditRefusal, WikitextEditor,
    WikitextEditorError, WikitextNodeDescriptor, WikitextNodeKind, WikitextNodeLocator,
    WikitextPageRef, normalize_anchor_text,
};
use sp42_parsoid::{editor_client, map_parsoid_error};
...
impl WikitextEditor for ParsoidWikitextEditor {
    ...
    async fn extract_blocks(
        ...
        sp42_parsoid::fetch_page_blocks(config, page).await
    }
}
...
<p><span about="#mwt1" typeof="mw:Transclusion" data-mw='{"parts":[{"template":{"target":{"wt":"lang","href":"./Template:Lang"},"params":{"1":{"wt":"fr"}},"i":0}},{"template":{"target":{"wt":"lang","href":"./Template:Lang"},"params":{"1":{"wt":"en"}},"i":1}}]}'>combined</span></p>
    ...
    fn maps_missing_page_error() {
        let mapped = sp42_parsoid::map_parsoid_error(parsoid::Error::PageDoesNotExist(
