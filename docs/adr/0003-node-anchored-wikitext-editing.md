...
6. **Implementation vehicle for the Parsoid impl: the `parsoid` crate** ([mwbot-rs](https://gitlab.wikimedia.org/repos/mwbot-rs/mwbot), Kunal Mehta; **GPL-3.0-or-later**). A maintained (47 releases; v0.11 Feb 2026) typed Rust wrapper over Parsoid HTML (Kuchiki-based) with **first-class `cite` / `Reference` / `ReferenceList` node types and a `Template` API (`filter_templates()`, `template.param("…")`)** — exactly our operations — fetching via `Client::get` (core REST `/w/rest.php`) and round-tripping via `Client::transform_to_wikitext`. Prefer it over a hand-rolled `reqwest` + `data-mw` JSON client. Caveats: (i) it is **GPL-3.0-or-later** — the same `deny.toml`/licensing action as the wikiparser-node option, **but Rust-native with no JS engine, so it does *not* incur ADR-0001 §1's "no-TypeScript" cost**; (ii) it still serializes via Parsoid REST (it is *not* an offline serializer) — and a source read (2026-06-04) found it uses `flavor=edit` HTML + revid, **not** the explicit stash/`If-Match` selective-serialization flow the raw spike proved, so its byte-level diff cleanliness is **unconfirmed** (see §Validation status); (iii) its `Wikicode` is `!Send` (Kuchiki/`Rc`) — use the provided `ImmutableWikicode` across `.await` points in the axum handler.
...
This decision is **provisional pending the acceptance gates in the pull request**. Evidence to date — recorded here because it is stable (it states what was tested and remains true):

- **Proven.** Param **modify/add** on an existing `{{cite …}}` round-trips **lossless + node-scoped + style-preserving** via the raw Parsoid REST transform with `?stash=true` + `If-Match` (method and diffs in the PR description). The one residual cost is light whitespace normalization *within the edited template only*.
- **Mechanism known, fidelity unconfirmed.** A source read of the `parsoid` crate (`parsoid/src/api.rs`) found it fetches `flavor=edit` HTML (inline `data-parsoid` present) and transforms via core REST **without** the explicit stash/`If-Match` selser flow — so its byte-level cleanliness is not yet proven equal to the raw spike's. If it under-preserves, the fallback (behind the trait, Decision 1) is to contribute selser upstream or pair the crate's manipulation with a bespoke stash/`If-Match` transform.
