...
- **The OAuth login precondition and redirect sanitization are untested.** The `412` returned when the confidential OAuth client is not configured, and the open-redirect guard on the callback's redirect target, have no verifying test.
- **`effective_session_scopes` derivation is not directly tested.** The mapping from a capability report to the `["basic","editpage","patrol","rollback"]` scope list is observed only transitively through the bootstrap test's asserted scope list; there is no unit test pinning the mapping itself.
