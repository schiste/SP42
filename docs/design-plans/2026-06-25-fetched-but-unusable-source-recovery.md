...
- `FetchedSource` in `verify.rs` — carry `content_type` through (already computed inside `fetch_source`, currently discarded).
...
**Components:**
- `NavChromePaywall` detector in `body_classifier.rs` — fires only when **both** a registration/paywall marker is present (curated phrase set, e.g. "subscribe to read", "sign in to continue", "create a free account", "register to continue") **and** the body lacks substantial contiguous article prose. The two-signal conjunction is the starting heuristic; it is tuned for balance per the net-value stance (see Additional Considerations), not toward a zero-false-positive corner.
