...
The verb surface is tiered by cost. `probe_source` is model-free and deterministic — it reports reachability, extractability, and the fetched-but-unusable reason so a caller can screen accessibility before paying for a panel call. `verify_claim` and the two convenience verbs run the model panel, declare MCP `taskSupport: "optional"` for the hosted phase, and execute synchronously over stdio in the MVP. A result cache keyed on `(claim, source-identity, panel_config_hash)` absorbs panel nondeterminism, making repeated calls idempotent without re-rolling. The MVP transport is local stdio with in-process `rmcp`-over-`sp42-core` (no double-hop); callers supply their own inference credentials via the existing `SP42_INFERENCE_*` env seam. The hosted streamable-HTTP transport, SSRF hardening for untrusted callers, and the roadmap verbs (`find_source`, `assess_reliability`) are out of scope for the MVP.
...
- Expensive verbs (`verify_claim` and the two convenience verbs) declare `taskSupport: "optional"` and run **synchronously over stdio for the MVP**, with the same handler wrappable as an async MCP task for the hosted phase — no forked logic. Identical `(claim, source, panel)` inputs return a **stably idempotent** verdict within the cache window (panel nondeterminism is absorbed by the cache, not re-rolled) — verified by a repeat-call test asserting a cache hit.
...
- **`idempotentHint`**: An MCP tool annotation signaling that identical inputs yield identical outputs within the cache window. Set on `verify_claim`; the result cache absorbs panel nondeterminism so the annotation is accurate and callers can safely replay or deduplicate calls.
...
assess_reliability(claim: string, url: string, wiki: string)
...
**Execution model.** Per MCP 2025-11-25 Tasks, expensive verbs declare `execution.taskSupport: "optional"`. Over stdio (MVP) the `rmcp` handler runs synchronously; the *same* handler is wrapped as an async task (`taskId` + poll) for the hosted phase, so there is one code path. Convenience verbs fan many `verify_claim` calls out and emit `notifications/tasks/status` progress ("18/40 use-sites verified") so even the synchronous MVP isn't opaque. A result cache keyed by `(claim, source-identity, panel_config_hash)` with a ~24h TTL makes repeat calls cheap (a BYO-key caller does not re-pay) and makes verdicts *stably* idempotent despite panel nondeterminism; `verify_claim` is annotated `idempotentHint: true`.
...
### Phase 6: Execution model — Tasks-optional, progress, idempotency cache
**Goal:** Make expensive verbs task-capable and cheap to repeat without forking logic.
...
- Result cache — keyed by `(claim, source-identity, panel_config_hash)`, ~24h TTL; `idempotentHint: true` on `verify_claim`.

**Dependencies:** Phase 5.

**Done when:** A repeat `verify_claim` call with identical inputs returns a cache hit (stable verdict, no re-roll); a `verify_wikipedia_page` run emits at least one progress event; tests pass.
