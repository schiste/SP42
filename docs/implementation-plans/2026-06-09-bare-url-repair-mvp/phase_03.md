# Bare-URL Repair MVP Implementation Plan — Phase 3: `bare_url_repair` core module

> **For Claude:** REQUIRED SUB-SKILL: Use ed3d-plan-and-execute:executing-an-implementation-plan to implement this plan task-by-task.

**Goal:** A pure, I/O-free `crates/sp42-core/src/bare_url_repair.rs` providing: bare-URL classification (PRD-0008 Resolved question 1), the metadata → citation-template renderer with full decline semantics, the propose/confirm wire types, and two small pure helpers for the shell (date formatting, Citoid language extraction). Checked-in Citoid fixture JSON covers the renderer matrix.

**Architecture (verified divergences from the design plan):**
- The lifted `CitoidMetadata` (Phase 1) is already *flattened*: `build_citoid_header` resolves `publicationTitle`→`websiteTitle` fallback into `publication`, formats `author`/`creators` arrays into one string, and **carries no language field**. The renderer therefore takes `language: Option<&str>` as a separate parameter, and a pure `citoid_language()` helper extracts it from the raw parsed object for the shell. (Note: the lifted code prefers `publicationTitle` over `websiteTitle` — the design plan stated the reverse order; the lifted code is the contract.)
- Reference anchors from `enumerate_nodes` are the **rendered text content** of the `<ref>` (Parsoid), not wikitext. For a bare-URL ref that text *is* the URL; for bracket-wrapped refs it is the (empty/label) link text — which is exactly why PRD Resolved question 1 excludes them, and why `classify_bare_url` works on anchor text directly.
- `replace_node` on a Reference replaces the ref's **contents**, preserving the `<ref>` element and its attributes (`name=` etc.). So `replacement_wikitext` is the bare template call (e.g. `{{cite web |…}}`) with **no** `<ref>` tags.
- Wire types live here in `sp42-core` (not in the server module as the design sketched) so the CLI (Phase 6) and server (Phases 4–5) share one serde contract — the `action_contracts` precedent.
- `iso_date_from_epoch_ms` keeps the core clock-free: the shell passes `clock.now_ms()` (Constitution Art. 1.4 — no `SystemTime::now()` in core).

**Tech Stack:** Rust, serde, serde_json; fixtures via `include_str!` from the workspace-root `fixtures/` directory (house convention). Clippy pedantic deny — public `fn`s need `#[must_use]`/doc comments in the house style shown below.

**Scope:** Phase 3 of 7. Depends on Phase 1 (`CitoidMetadata`); Phase 2 recommended first (config field exists) but not required to compile.
...
**Types you will use (already exist after Phase 1):**

```rust
// crates/sp42-core/src/wikitext_editor.rs
pub enum WikitextNodeKind { Reference, Template }          // serde kebab-case
pub struct WikitextNodeLocator { pub kind: WikitextNodeKind, pub ordinal: usize, pub expected_text: String }
pub struct WikitextNodeDescriptor { pub kind: WikitextNodeKind, pub ordinal: usize, pub anchor_text: String }

// crates/sp42-core/src/citation/citoid.rs (lifted)
pub struct CitoidMetadata {
    pub publication: Option<String>,  // publicationTitle, fallback websiteTitle
    pub published: Option<String>,    // "date", ISO with partial precision
    pub author: Option<String>,       // pre-formatted "Jane Doe, John Smith"
    pub title: Option<String>,
    pub url: String,                  // echoed source URL
}
...
### Task 1: Check in the Citoid fixtures

**Files:**
- Create: `fixtures/citoid/basic.json`
- Create: `fixtures/citoid/degenerate_title_url.json`
- Create: `fixtures/citoid/partial_date.json`
- Create: `fixtures/citoid/no_title.json`
- Create: `fixtures/citoid/creators_fallback.json`

These replicate the response shapes the lifted parser was written against (Citoid `mediawiki-basefields`: a JSON **array** of citation objects; `author` entries are `[first, last]` arrays; the degenerate scrape-failure case echoes the URL as `title` with HTTP 200/520).

**Step 1: Create the files**

`fixtures/citoid/basic.json` — the normal full-fields case:
...
`fixtures/citoid/degenerate_title_url.json` — Citoid's documented scrape-failure fallback (URL echoed as title):

```json
[
  {
    "itemType": "webpage",
    "title": "https://degenerate.example/x",
    "url": "https://degenerate.example/x"
  }
]
```

`fixtures/citoid/partial_date.json` — partial-precision date plus the `websiteTitle` fallback path:
...
`fixtures/citoid/no_title.json` — metadata present but no usable title:
...
`fixtures/citoid/creators_fallback.json` — the `creators` author fallback:
...
```bash
for f in fixtures/citoid/*.json; do python3 -m json.tool "$f" > /dev/null && echo "ok: $f"; done
```
...
```bash
git add fixtures/citoid/
git commit -m "test: add Citoid response fixtures for bare-URL repair"
```

### Task 2: Classification — `classify_bare_url` and `bare_url_references`

**Files:**
- Create: `crates/sp42-core/src/bare_url_repair.rs`
- Modify: `crates/sp42-core/src/lib.rs` (module decl; re-exports come in Task 4)

**Step 1: Write the failing tests**

Create `crates/sp42-core/src/bare_url_repair.rs` with the module skeleton and the classification tests (implementation stubs come next step — write tests first inside the file, then run to see them fail to compile):
...
Also declare the module in `crates/sp42-core/src/lib.rs` — in the module-declaration block, between `article_inventory` and `branding`:
...
```bash
cargo test -p sp42-core bare_url_repair
```

Expected: compile errors — `classify_bare_url`, `bare_url_references`, `BareUrlReference` not found.
...
(Import discipline: `warnings = deny` makes unused imports a build failure, so the file starts with only `use crate::wikitext_editor::{WikitextNodeDescriptor, WikitextNodeKind};`. Task 3 extends the use-list with `serde::{Deserialize, Serialize}`, `serde_json::{Map, Value}`, and `crate::citation::citoid::CitoidMetadata`; Task 4 adds `WikitextNodeLocator` to the `wikitext_editor` import — each exactly when first used.)
...
```bash
git add crates/sp42-core/src/bare_url_repair.rs crates/sp42-core/src/lib.rs
git commit -m "feat: add bare-URL reference classification"
```

### Task 3: Renderer with decline semantics

**Files:**
- Modify: `crates/sp42-core/src/bare_url_repair.rs`

**Step 1: Write the failing tests**

Add to the `tests` module (these drive the full fixture pipeline: `parse_citoid_response` → `build_citoid_header` → `citoid_language` → `render_bare_url_citation`):
...
(The expected output is a single line: `{{<template> ` followed by `|name=value` segments separated by single spaces, closed by `}}`.)
...
Expected: compile errors — `BareUrlOutcome`, `BareUrlDeclineReason`, `render_bare_url_citation`, `citoid_language` not found.

**Step 3: Implement the renderer**

Add to `bare_url_repair.rs` (above the tests; extend the `use` list with `serde::{Deserialize, Serialize}`, `serde_json::{Map, Value}`, and `crate::citation::citoid::CitoidMetadata` as they now have users):
...
Expected: all renderer tests pass (plus the 3 classification tests).

**Step 5: Commit**

```bash
git add crates/sp42-core/src/bare_url_repair.rs
git commit -m "feat: render bare-URL citations from Citoid metadata"
```

### Task 4: Wire types, date helper, and re-exports

**Files:**
- Modify: `crates/sp42-core/src/bare_url_repair.rs`
- Modify: `crates/sp42-core/src/lib.rs`
...
Expected: compile errors — `iso_date_from_epoch_ms`, `BareUrlProposal`, `BareUrlProposalsResponse`, `BareUrlDeclined` not found.

**Step 3: Implement**

Add to `bare_url_repair.rs`:
...
In `crates/sp42-core/src/lib.rs`, insert between the `article_inventory` and `citation` re-export blocks (ASCII-sorted names, house style):
...
Expected: all module tests pass. Double-check the date assertions: `951_782_400_000` ms is 2000-02-29T00:00Z (leap day) and `1_780_963_200_000` ms is 2026-06-09T00:00Z. If `iso_date_from_epoch_ms` disagrees, fix the implementation, not the test (cross-check: `python3 -c "import datetime;print(datetime.datetime.fromtimestamp(1780963200, datetime.timezone.utc))"`).
...
Expected: green. (If clippy pedantic flags `cast` or arithmetic style in `iso_date_from_epoch_ms`, prefer adding the narrowly-scoped `#[allow(clippy::many_single_char_names)]`-style fix it actually asks for — but only with the lint name it reports; do not blanket-allow.)

**Step 7: Commit**

```bash
git add crates/sp42-core/src/bare_url_repair.rs crates/sp42-core/src/lib.rs
git commit -m "feat: add bare-URL repair wire types and date helper"
```
